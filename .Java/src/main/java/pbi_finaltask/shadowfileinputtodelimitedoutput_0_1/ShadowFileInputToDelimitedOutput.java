...
package pbi_finaltask.shadowfileinputtodelimitedoutput_0_1;
...
public class ShadowFileInputToDelimitedOutput implements TalendJob {
	...
	public static class RowStruct implements routines.system.IPersistableRow<RowStruct> {
		final static byte[] commonByteArrayLock_PBI_FINALTASK_ShadowFileInputToDelimitedOutput = new byte[0];
		static byte[] commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput.length) {
					if (length < 1024 && commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput.length == 0) {
						commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput = new byte[1024];
					} else {
						commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput, 0, length);
				strReturn = new String(commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput, 0, length);
				strReturn = new String(commonByteArray_PBI_FINALTASK_ShadowFileInputToDelimitedOutput, 0, length,
						utf8Charset);
			}
			return strReturn;
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PBI_FINALTASK_ShadowFileInputToDelimitedOutput) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				fileName_tFileOutputDelimited = (new java.io.File(
						"D:/PBI-FinalTask/PBI_FINALTASK/temp/preview/transaction_excel.csv")).getAbsolutePath()
								.replace("\\", "/");
				...
				final String decryptedPassword_tFileInputExcel = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:4CZt8m5jIAkbi8rwOHsggKmTQ48P2W2Pt5g3Og==");
				...
				Object source_tFileInputExcel = "D:/PBI-FinalTask/Dataset/transaction_excel.xlsx";
				...
				if (source_tFileInputExcel instanceof String) {
					workbook_tFileInputExcel = (org.apache.poi.xssf.usermodel.XSSFWorkbook) org.apache.poi.ss.usermodel.WorkbookFactory
							.create(new java.io.File((String) source_tFileInputExcel), password_tFileInputExcel, true);
				...
				try {
					...
					if (sheetList_tFileInputExcel.size() > 0) {
						...
						for (int i_tFileInputExcel = begin_line_tFileInputExcel; i_tFileInputExcel < end_line_tFileInputExcel; i_tFileInputExcel++) {
							...
							Row = null;
							int tempRowLength_tFileInputExcel = 6;
							...
							try {
								...
								globalMap.put("tFileInputExcel_ERROR_MESSAGE", e.getMessage());
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = ShadowFileInputToDelimitedOutput.class.getClassLoader().getResourceAsStream(
					"pbi_finaltask/shadowfileinputtodelimitedoutput_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
...
/************************************************************************************************
 * 57651 characters generated by Talend Open Studio for Data Integration on the
 * December 29, 2024 at 2:33:01 AM WIB
 ************************************************************************************************/